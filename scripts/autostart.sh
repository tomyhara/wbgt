...
# 設定ファイルの存在確認
if [ ! -f "../setup/config.py" ]; then
    echo "$(date): ❌ エラー - config.py が見つかりません" >> "$LOG_FILE"
    echo "$(date): setup/config.sample.py をコピーして setup/config.py を作成してください" >> "$LOG_FILE"
    exit 1
fi
...
if ! python3 -c "import requests" &> /dev/null; then
    ...
    pip3 install -r ../setup/requirements.txt >> "$LOG_FILE" 2>&1
fi
...
python3 ../src/wbgt_kiosk.py >> "$LOG_FILE" 2>&1
